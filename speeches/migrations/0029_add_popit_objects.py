...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for speaker in orm['speeches.Speaker'].objects.all():
            if speaker.popit_url:
                api_url = re.sub('(api/v[0-9.]+/).*', r'\1', speaker.popit_url)
                api_instance, _ = orm['popit.ApiInstance'].objects.get_or_create(url=api_url)
                person, _ = orm['popit.Person'].objects.get_or_create(
                    api_instance = api_instance,
                    popit_url = speaker.popit_url,
                    name = speaker.name,
                )
    ...
    def backwards(self, orm):
        for speaker in orm['speeches.Speaker'].objects.all():
            if speaker.person:
                speaker.popit_url = speaker.person.popit_url
                speaker.person = None
                speaker.save()
