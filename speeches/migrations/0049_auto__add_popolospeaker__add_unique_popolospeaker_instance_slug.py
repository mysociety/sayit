...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'PopoloSpeaker'
        db.create_table('speeches_popolospeaker', (
            ('person_ptr', self.gf('django.db.models.fields.related.OneToOneField')(to=orm['popolo.Person'], unique=True, primary_key=True)),
            ('instance', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['instances.Instance'])),
            ('slug', self.gf('sluggable.fields.SluggableField')(unique_with=('instance',), max_length=50, populate_from='name')),
        ))
        db.send_create_signal('speeches', ['PopoloSpeaker'])

        # Adding unique constraint on 'PopoloSpeaker', fields ['instance', 'slug']
        db.create_unique('speeches_popolospeaker', ['instance_id', 'slug'])


    def backwards(self, orm):
        # Removing unique constraint on 'PopoloSpeaker', fields ['instance', 'slug']
        db.delete_unique('speeches_popolospeaker', ['instance_id', 'slug'])

        # Deleting model 'PopoloSpeaker'
        db.delete_table('speeches_popolospeaker')
