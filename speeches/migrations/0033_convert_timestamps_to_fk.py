...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for rt in orm['speeches.RecordingTimestamp'].objects.all():
            recording_set = rt.recording_set
            if recording_set.count() == 0:
                raise Exception("No recording!")
            if recording_set.count() > 1:
                raise Exception("Actual M2M data, eeeek!")
            rt.recordingN = recording_set.all()[0]
            rt.save()

    def backwards(self, orm):
        for rt in orm['speeches.RecordingTimestamp'].objects.all():
            rt.recording_set.add(rt.recordingN)
            rt.recordingN = None
            rt.save()
